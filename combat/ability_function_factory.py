...
class AbilityFunctionFactory:
    ...
    def heavy_blow_effect(self, dam_min, dam_max, base_stun_chance):
        def out(c: Creature, t: Creature):
            ...
            if t.is_alive():
                roll = random() * 100
                if roll < base_stun_chance - t.endurance * 5 + c.strength * 5:
    ...
    def flickering_flames_attack(self, base_hit_chance):
        def out(c: Creature, t: Creature):
            roll = random() * 100
            success = roll < base_hit_chance - t.dodge * 5 + c.intelligence * 5
            if not success:
    ...
    def flickering_flames_effect(self):
        def out(c: Creature, t: Creature):
            ...
            t.add_effect(self.effects.create_fire_effect(c.intelligence, 2 + c.intelligence))
        return out
